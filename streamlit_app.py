"""
UWAGA:
Upewnij się, że w Twoim repozytorium znajduje się folder `.streamlit` z plikiem `config.toml`
o następującej treści, aby wyłączyć mechanizm obserwatora zmian w plikach:
...
def calculate_contrast_ratio(color1, color2):
    """Oblicza współczynnik kontrastu WCAG między dwoma kolorami."""
...
def check_wcag_compliance(contrast_ratio):
    """Sprawdza zgodność kontrastu z wymaganiami WCAG."""
    return {
...
def load_ocr_reader(languages=['pl', 'en']):
    ...
    st.write("Inicjalizacja modelu EasyOCR – może to potrwać kilka minut...")
    try:
        # Import torch dopiero tutaj, aby uniknąć problemów przy uruchomieniu
        import torch
    ...
    try:
        ...
        st.write("Model EasyOCR został poprawnie załadowany.")
        return reader
    except Exception as e:
...
def detect_text_easyocr(image, reader):
    """Wykonuje detekcję tekstu w obrazie za pomocą EasyOCR."""
    try:
...
def sample_colors_from_region(image, region, num_samples=10):
    """Analizuje kolory danego regionu i przypisuje tło oraz kolor tekstu."""
...
def check_edge_distance(image_shape, region, margin_h_px, margin_v_px):
    """Sprawdza odległość regionu od krawędzi obrazu."""
    try:
        x1, y1, x2, y2 = region['bbox']
...
def visualize_results(original_image, regions_with_analysis, margin_h_px, margin_v_px):
    """Tworzy wizualizację analizy z nakładką graficzną oraz dodaje szczegóły (kontrast, odległości)."""
    try:
        ...
        # Nakładanie oznaczeń dla wykrytych regionów
        for region_data in regions_with_analysis:
            ...
            if compliance['AAA_normal']:
                box_color = [0, 255, 0, 200]  # Zielony
            elif compliance['AA_normal']:
                box_color = [0, 0, 255, 200]  # Niebieski
            else:
                box_color = [255, 0, 0, 200]  # Czerwony
        ...
        # Dodanie informacji o współczynniku kontrastu
        for region_data in regions_with_analysis:
            region = region_data['region']
            color_data = region_data['colors']
            if not color_data:
...
def main():
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            st.error(f"Błąd podczas wczytywania obrazu: {e}")
            return
        ...
        if reader is None:
            st.error("Nie udało się załadować modelu OCR.")
            return
